class Numberline extends Widget {
    ...
    preinit() {
        ...
        // Fetch level data from filesystem
        core.filesystem.readFile(this.fullPath + "/levels.json")
            .then(levels => {
                this.leveldata = JSON.parse(levels);
            });
    ...
    initialize() {
        this.levelHandler = new NumberlineLevelHandler(this.leveldata);
    }
...
class NumberlineSlider {
    ...
    initializeSlider() {
        ...
        this.sliderTouchEvent.on('press', (e) => { e.preventDefault(); this.enterPad(); });
        this.sliderTouchEvent.on('pressup', (e) => { this.releasePad(); });
    ...
    tapPad() {
        this.animPadDown();
        setTimeout(() => { this.releasePad() }, 200);
    }
    enterPad() {
    ...
    animPadUp() {
        this.slider_pad.classList.remove("push");
    }

    animPadDown() {

        this.slider_pad.classList.add("push");
    }
    ...
    setBlocks(numberOfBlocks) {
        ...
        let padPositionX = (0.076 * this.width) + ((this.width - (0.10 * this.width)) / this.range.total * this.numberOfBlocks + 1);
        let blockWidth = (this.block_holder.clientWidth - (0.005 * this.block_holder.clientWidth)) / this.range.total;
        this.block_holder.innerHTML = "";

        for (let i = 0; i < this.numberOfBlocks; i++) {
            this.block_holder.innerHTML += '<div class="block" style="width:'+blockWidth+'px"><div class="img"></div></div>';
        }
    ...
    createSeparators() {
        ...
        for (let i = 0; i < this.range.total + 1; i++) {
            ...
            let xPosition = (this.width * 0.05) + ((this.width * 0.9) / this.range.total * i);
...
class NumberlineLevel {
    ...
    showHelpingArrowTo(from, to) {
        ...
        if(this.submittedAnswers.length > 0) {
            if(this.problem.operators[this.submittedAnswers.length - 1] === "-") {
                forward = false;
            }
        }
        ...
        if(forward) {
            ...
            left = ((this.slider.width * 0.9) / this.slider.range.total * ((from-to)+1));
            width = ((this.slider.width * 0.9) / this.slider.range.total * to);
    ...
    displayProblem() {
        ...
        for(let i = 0; i < this.problem.operands.length; i++) {
            if(i == 0) {
                this.current_problem.innerHTML += '<h1 class="operand active">'+this.problem.operands[i]+'</h1>'
            } else {
                this.current_problem.innerHTML += '<h1 class="operand">'+this.problem.operands[i]+'</h1>'
            }
            if(this.problem.operators.length > 1) 
                this.current_problem.innerHTML += '<h1 class="operator">'+this.problem.operators[i]+'</h1>'
        }
    ...
    nextProblem() {
        ...
        document.getElementById("level_indicator_bg").style.transform = 'scale('+baseScale+','+baseScale+')';
        ...
        if(this.replays < 7) {
            ...
            setTimeout(() => {
                ...
                document.getElementById("level_indicator_bg").style.transform = 'scale('+1+','+1+')';
            }, 2000);
        }
    ...
    submitAnswer(answer) {
        ...
        if(this.submittedAnswers.length === 0 && parseInt(this.problem.result) === parseInt(answer)) {
            let displayOperands = document.getElementsByClassName("operand");
            displayOperands[this.submittedAnswers.length].classList.remove("active");
        ...
        if(parseInt(this.problem.answerChain[this.submittedAnswers.length]) === parseInt(answer)) {
            let displayOperands = document.getElementsByClassName("operand");
            displayOperands[this.submittedAnswers.length].classList.remove("active");

            if(this.submittedAnswers.length === this.problem.answerChain.length - 1) {
                ...
                if(this.help.showArrow)
                    this.showHelpingArrowTo(answer, this.problem.operands[this.submittedAnswers.length]);
                
                this.slider.setBlocks(answer);
                if(this.submittedAnswers.length < this.problem.answerChain.length)
                    displayOperands[this.submittedAnswers.length].classList.add("active");
    ...
    parseProblem(problem) {
        ...
        .forEach(group => {
            ...
            if (operator !== "=" && operator.length > 1) {
                operator = operator[NumberlineLevel.randomRange(0, operator.length) -1]
            }
            
            let randomMax = parseInt(operand.split("-")[1]);
            let randomMin = parseInt(operand.split("-")[0]);

            operand = Math.floor(Math.random() * (Math.floor(randomMax) - Math.floor(randomMin)) + Math.floor(randomMin));
            ...
            switch(operator) {
                ...
                case "+":
                    if(result + operands[i+1] > this.range.end) {
                        operands[i+1] = operands[i+1] % (this.range.end - result);
                    }
                    result += operands[i+1];
                    break;
            }
    ...
    parseRange(range) {
        ...
        if(range.charAt(0) === "-") {
            ...
            total = Math.abs(parseInt(rangeSplitted[1]) + parseInt(rangeSplitted[2]))
        } else {
            start = parseInt(rangeSplitted[0])
            end = parseInt(rangeSplitted[1])
            total = Math.abs(parseInt(rangeSplitted[0]) + parseInt(rangeSplitted[1]))
        }
    ...
    static randomRange(x, y) {
        return Math.abs(Math.floor(Math.random() * (Math.ceil(x) - Math.floor(y))) + Math.ceil(x));
    }
}